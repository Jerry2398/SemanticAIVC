...
#PBS -j oe
#PBS -o logs/tahoe_vae_nb.out

# Train the NB-likelihood VAE on Tahoe-100M (streaming) + validate.
# A/B vs the Gaussian VAE (tahoe_vae): same backbone/latent/budget, NB observation model.
#   qsub -v LATENT=32,MAX_STEPS=532000,EVAL_EVERY=10000 submit_vae_nb.sh
...
python -u train_nb.py --latent_dim "$LATENT" --max_steps "$MAX_STEPS" \
    --batch_size 512 --num_workers 8 --eval_every "$EVAL_EVERY"
python -u evaluate_nb.py --max_batches 200
echo "done: NB-VAE ckpt + metrics.csv + loss_curve.png in /scratch/.../models/tahoe_vae_nb"
