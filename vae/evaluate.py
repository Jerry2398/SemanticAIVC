"""
Standalone validation of a trained Tahoe VAE: reconstruction of val HVG
expression (gene/cell Pearson, R2, MSE) + a generative mean-profile check.

  python evaluate.py --max_batches 100
"""
import argparse
import json
import os
...
def main():
    ...
    a = ck["args"]
    model = SimpleVAE(ck["n_genes"], a["latent_dim"], a["hidden"], a["depth"]).to(dev)
    ...
    m.update(g); m.update({"n_val_cells": int(X.shape[0]), "step": ck.get("step")})
    print("=== Tahoe VAE validation ===")
    for k, v in m.items():
